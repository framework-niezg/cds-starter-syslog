...
export default class Util {
    ...
    static resetParamEq(params){
      if(params.query && params.query.length>0){
        ...
        params.query.map((param)=>{//name,role
          if(params[param]&&params[param]!==""){
            if(param=="createTime"){
              params.queryString.push(param+"~Ge~"+params[param][0]);
              params.queryString.push(param+"~Le~"+(params[param][1]+86399999));
            }else{
              params.queryString.push(param+"~Eq~"+params[param])
            }
    ...
    static copyToClipboard() {
      var Url2=document.getElementById("copyToken");
      Url2.select(); // 选择对象
