<template>
  <el-card class="box-card scrollbar">
    ...
    <el-table v-if="current" ref="basic" :data="tableData[current]"
    ...
    >
        ...
        <template scope="scope">
          <template v-for="deal in scope.row.deals">
            ...
            <el-button v-else
                       :type="deal.type"
                       size="mini"
                       :data-clipboard-action="deal.action"
                       :class="deal.className"
                       v-clipboard:copy="'用户名:'+ scope.row.name +',授权码:'+scope.row.token"
      ...
      <el-table-column v-if="options[current].isExpand=='true'" type="expand" width="80">
        <template slot-scope="props">
          <el-form v-if="options[current].expandLabel" label-position="left" inline class="table-expand">
            <el-form-item  v-for="item in options[current].expandLabel" :label="item.label">
              <span>{{ props.row[item.property] }}</span>
            </el-form-item>
          </el-form>
          <span v-else>没数据</span>
        </template>
      </el-table-column>
    </el-table>
  ...
  export default {
    ...
    methods: {
      onCopy:function(e){
        ...
        if(this.currentTable=="apiManager"){
          this.message("info","用户授权码已经复制到剪切板!");
        }else{
          console.log(e.text);
        }
      },
      ...
      toggleSelection(row) {
        if (row) {
          this.$refs.basic.clearSelection();
          this.$refs.basic.toggleRowSelection(row);
        } else {
          this.$refs.basic.clearSelection();
      ...
      setCurrentRow(){
        this.$refs.basic.setCurrentRow(this.tableData[this.current][0]);
      },
      ...
      handleCopy({name,account,token}){

        this.message("info","您复制了用户"+ name +"授权码："+token);
      },
      handleChange(row) {
        //this.toggleSelection(row);
